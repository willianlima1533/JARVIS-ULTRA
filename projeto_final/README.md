...
*   **Reconhecimento de Fala**: Suporta VOSK (offline) e Whisper para capturar comandos de voz
*   **Síntese de Voz (TTS)**: Utiliza termux-tts-speak (nativo), pyttsx3 (offline) ou gTTS (online) para feedback por voz
...
3. **Descompacte o projeto**:
   ```bash
   cd ~
   unzip ~/storage/downloads/projeto_final.zip -d ~/projects
...
2. **Dê permissões de execução**:
   ```bash
   chmod -R +x ~/projects/projeto_final/scripts
   ```
...
4. **Instale as dependências**:
   ```bash
   bash ~/projects/projeto_final/scripts/install_termux.sh
   ```
5. **Inicie o assistente**:
   ```bash
   bash ~/projects/projeto_final/scripts/start_assistente.sh
   ```
...
*   **Termux TTS**: Para melhor experiência de voz, instale o Termux:API da F-Droid.
...
**Voz não funciona:**
- Verifique se termux-tts-speak está instalado: `pkg install termux-api`
